...
register uint32_t SCTLR __asm("cp15:0:c1:c0:0");		// コプロセッサ-システムコントロールレジスタ, 19/11/14
...
static inline __attribute__((always_inline)) void SCTLR_WRITE(uint32_t val)
{
	SCTLR = val;
}
#define SCTLR_M (1<<0)		// MMU許可ビット
#define SCTLR_A (1<<1)		// 境界検知ビット
#endif
...
register uint32_t TTBR0 __asm("cp15:0:c2:c0:0");		// コプロセッサ-変換テーブルベースレジスタ0, 19/11/14
static inline __attribute__((always_inline)) void TTBR0_WRITE(uint32_t val)
{
	TTBR0 = val;
}

/* Domain Access Control Register */
register uint32_t DACR __asm("cp15:0:c2:c0:0");		// コプロセッサ-ドメインアクセスコントロールレジスタ, 19/11/14
static inline __attribute__((always_inline)) void DACR_WRITE(uint32_t val)
{
	DACR = val;
}
#endif
...
{
#if 0
	// enable cache/TLB maintenance broadcast/L1 prefetch/L2 prefetch, 19/04/22
	ACTLR_WRITE(ACTLR_READ() | ACTLR_SMP | ACTLR_FW | ACTLR_L1 | ACTLR_L2);
#else
...
#elif 1
	intptr_t __cs3_region_start_boot_rom = (intptr_t)BOOT_ROM_BASE;
	intptr_t __cs3_region_end_ram = (intptr_t)SDRAM_END;
	intptr_t _no_mem_start = (intptr_t)NO_MEM_BASE;
	intptr_t _no_mem_size = (intptr_t)NO_MEM_SIZE;
	intptr_t _peripherals_start = (intptr_t)PERIPHERAL_BASE;
	intptr_t _peripherals_size = (intptr_t)PERIPHERAL_SIZE;
	UINT32 start = 0;
	UINT32 size = 0;
	
	
	// SDRAMエリアのキャッシュ属性
	start = __cs3_region_start_boot_rom;
	size = __cs3_region_end_ram - start;
	mmu_tbl_set(tx_alt_ttb[core], start, size, (MMU_CB_SHARED | MMU_CB_WBACK | MMU_CB_TEX0_WALLOC));
	
	// デバイス無しエリアのキャッシュ属性
	start = _no_mem_start;
	size = _no_mem_size;
	mmu_tbl_set(tx_alt_ttb[core], start, size, MMU_CB_NONE);
	
	// 内蔵モジュールエリアのキャッシュ属性
	start = _peripherals_start;
	size = _peripherals_size;
	mmu_tbl_set(tx_alt_ttb[core], start, size, MMU_CB_BUFF);
#endif
}

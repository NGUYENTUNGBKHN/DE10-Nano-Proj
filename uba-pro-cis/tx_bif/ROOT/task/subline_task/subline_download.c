...
extern QSPI_FLASH_HANDLE hFlash;
#endif
...
u8 _write_rom(u8 *pdata, u32 len)
{
	...
	qspi_buf.len = len;
	qspi_buf.byte_count = &i;
	if( QSPI_Flash_Write( &hQFlash, &qspi_buf, SPI_FLASH_ERASE_AUTO ) == FALSE ){
		DBG_ERR();
		return(FALSE);
	};
...
static void _dl_cmd_analyze(void)
{
	if(_dl_verify_command_format())
	{
		...
		case CMD_ID_DNLOAD_START:
			...
			_dl_cmdD1();
			//2023-12-22 subラインからのメッセージは現状ないので、ダウンロード書き込み中のLED点灯は保留
			//_main_send_msg(ID_DISPLAY_MBX, TMSG_DISP_LED_DOWNLOAD_ING, 0, 0, 0, 0);
			break;

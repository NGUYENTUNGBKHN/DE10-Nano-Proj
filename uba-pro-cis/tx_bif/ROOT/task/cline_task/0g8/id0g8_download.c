...
void Fif_led_set( u8 data ) //2022-07-14
{
...
#if 0//#if !defined(ID0G8_BOOTIF) // boot ifはすでに別ファイルで存在する
...
void ProcDfuManifestState(void)		/* 0x0F*//* 書き込み後のFlash CRC確認 */
{
	...
	case DFU_STATUS_ERR_VERIFY:
		...
		_cline_send_msg(ID_DISPLAY_MBX, TMSG_DISP_LED_ALARM, ALARM_CODE_IF_AREA, ex_operating_mode, 0, 0); /* ROM check IF-AREA error */
...
void tetsuya_line(u8 type, u8 state)
{
	...
	if(iwasaki_seq_count_out != 0 && iwasaki_seq_count_out < (IWASAKI_COUNT_MAX-1))
	{
		iwasaki_seq_type_out[iwasaki_seq_count_out] = type;
		iwasaki_seq_state_out[iwasaki_seq_count_out] = state;
		iwasaki_seq_count_out++;
	}
	iwasaki_line_count++;
}
#endif

/******************************************************************************/
/*! @addtogroup Group2
    @file       id003.c
    @brief      id003 process
    @date       2013/03/22
    @author     Development Dept at Tokyo
    @par        Revision
    $Id$
    @par        Copyright (C)
    2012-2013 Japan CashMachine Co, Limited. All rights reserved.
*******************************************************************************
    @par        History
    - 2013/03/22 Development Dept at Tokyo
      -# Initial Version
******************************************************************************/
#include <string.h>
#include "kernel.h"
#include "itron.h"
#include "common.h"
#include "custom.h"
#include "id003.h"
#include "js_uart_id003.h"
#include "memorymap.h"
#include "sub_functions.h"
#include "id003.h"
#include "download.h"

#define EXT
#include "com_ram.c"

/************************** PRIVATE DEFINITIONS *************************/
/************************** PRIVATE VARIABLES *************************/
ID003_CMD_BUFFER _id003_rx_buff;
ID003_CMD_BUFFER _id003_tx_buff;
static u8 s_abnomal_code; /**< @brief Abnormal data */

static u8 s_id003_communication_stsreq_flag;


/************************** PRIVATE FUNCTIONS *************************/
static void _init_variable( void );
void _id003_initial_msg_proc(void);
void _id003_msg_proc(void);

void _id003_cmd_proc(void);

/* Status request */
void _id003_sts_request_cmd_proc(void);
void _id003_stsreq_failure(void);
void _id003_stsreq_write_rom_failure(void);
void _id003_stsreq_download_error(void);
void _id003_stsreq_download_end(void);
void _id003_stsreq_download(void);
void _id003_send_host_commu_err(void);

/* Operation command */
void _id003_download_start_cmd_proc(void);
void _id003_download_end_confirm_cmd_proc(void);
void _id003_download_baudrate_cmd_proc(void);
void _id003_diff_download_start_cmd_proc(void);
void _id003_download_data_cmd_proc(void);
void _id003_ack_proc(void);

/* Response */
void _id003_send_host_ack(void);
void _id003_send_host_invalid(void);

/* MAIN status message */
void _id003_status_info_msg_proc(void);
/* UART1 callback message */
void _id003_callback_msg_proc(void);
void _id003_callback_receive(void);
void _id003_callback_empty(void);
void _id003_callback_alarm(void);

void _id003_write_flash_rsp_msg_proc(void);
void _id003_download_calc_crc_rsp_msg_proc(void);
void _id003_download_rsp_msg_proc(void);

/************************** EXTERN VARIABLES *************************/
T_MSG_BASIC cline_msg;

/************************** EXTERN FUNCTIONS *************************/
extern void _cline_send_msg(u32 receiver_id, u32 tmsg_code, u32 arg1, u32 arg2, u32 arg3, u32 arg4);
extern void _cline_system_error(u8 fatal_err, u8 code);


ID003_CMD_BUFFER _id003_rx_buff;
ID003_CMD_BUFFER _id003_tx_buff;

#if 1 //2023-12-22
u8 ex_download_start_id003;
#endif


/*******************************
        line_task
 *******************************/
u8 id003_main(void)
{
	T_MSG_BASIC *tmsg_pt;
	ER ercd;

	_id003_initial_msg_proc();

	while (1)
	{
		ercd = trcv_mbx(ID_CLINE_MBX, (T_MSG **)&tmsg_pt, TASK_WAIT_TIME);
		if (ercd == E_OK)
		{
		/* Receiving task message */
			memcpy(&cline_msg, tmsg_pt, sizeof(T_MSG_BASIC));
			if ((rel_mpf(cline_msg.mpf_id, tmsg_pt)) != E_OK)
			{
				/* system error */
				_cline_system_error(1, 3);
			}
 			_id003_msg_proc();
		}

		// if download start flag set, TX disabled and UART not busy
		if( (ex_cline_download_control.is_end_sent)
		 && (UART_is_txd_active()  == 0) )
		{
	    	_cline_send_msg(ID_MAIN_MBX, TMSG_CLINE_SOFT_RESET_REQ, 0, 0, 0, 0);
			ex_cline_status_tbl.line_task_mode = ID003_MODE_QUIT;
		}
		if(ex_cline_status_tbl.line_task_mode == ID003_MODE_QUIT)
		{
			break;
		}
	}
	return 0;
}

void id_abn_set(int code)
{
	s_abnomal_code = code;
}
void _id003_initial_msg_proc(void)
{
	T_MSG_BASIC *tmsg_pt;
	ER ercd;
	u8 wait_init = 1;
	/* clear abnormal code */
	s_abnomal_code = 0;

#if 1 //2023-12-22
	ex_download_start_id003 = 0;
#endif

	/* Wait TMSG_CLINE_INITIAL_REQ */
	while (wait_init)
	{
		ercd = rcv_mbx(ID_CLINE_MBX, (T_MSG **)&tmsg_pt);
		if (ercd == E_OK)
		{
			memcpy(&cline_msg, tmsg_pt, sizeof(T_MSG_BASIC));
			memset(tmsg_pt, 0, sizeof(T_MSG_BASIC));
			if ((rel_mpf(cline_msg.mpf_id, tmsg_pt)) != E_OK)
			{
				/* system error */
				_cline_system_error(1, 4);
			}
			if ((cline_msg.tmsg_code == TMSG_CLINE_INITIAL_REQ))
			{
				if ((cline_msg.arg2 != TMSG_SUB_SUCCESS) && (cline_msg.arg2 != TMSG_SUB_ALARM))
				{
					/* system error ? */
					_cline_system_error(0, 5);
				}
				else
				{
					wait_init = 0;
					if (cline_msg.arg1 & OPERATING_MODE_IF_DOWNLOAD)
					{
					    /* ダウンロード関係変数の初期化 */
					    _init_variable();
					    /* ダウンロード開始要求フラグ     */
						ex_cline_download_control.is_start_received = DL_START_NORMAL;
					    /* Download Mode */
						ex_cline_status_tbl.line_task_mode = ID003_MODE_DOWNLOAD;
					}
					else if(cline_msg.arg1 & OPERATING_MODE_IF_DIFF_DOWNLOAD)
					{
					    /* ダウンロード関係変数の初期化 */
					    _init_variable();
					    /* ダウンロード開始要求フラグ     */
						ex_cline_download_control.is_start_received = DL_START_DIFFERENTIAL;
					    /* Download Mode */
						ex_cline_status_tbl.line_task_mode = ID003_MODE_DOWNLOAD;

					}
					else
					{
					    /* ダウンロード関係変数の初期化 */
					    _init_variable();
					    /* ダウンロード開始要求フラグ     */
						ex_cline_download_control.is_start_received = DL_START_NOT;
					/* Wait Command Mode */
						ex_cline_status_tbl.line_task_mode = ID003_MODE_WAIT_REQ;
						id_abn_set(ALARM_CODE_IF_AREA);
					}
				}
			}
		}
	}


	s_id003_communication_stsreq_flag = 0;

	/* --------------------------------------------------------------------- */
	/* Start id-003 communication                                            */
	/* --------------------------------------------------------------------- */
	/* Initialize UART */
	uart_init_id003();
	/* wakeup ID_UART01_CB_TASK */
	act_tsk(ID_UART01_CB_TASK);
}


/*------------------------------------------------------*/
/*  変数の初期化処理                                                                                         */
/*  <引数> なし                                                                                                   */
/*  <返値> なし                                                                                                   */
/*------------------------------------------------------*/
void static _init_variable( void )
{
	// 変数初期化
	memset(&_id003_rx_buff, 0, sizeof(_id003_rx_buff));
	memset(&_id003_tx_buff, 0, sizeof(_id003_tx_buff));
	memset(&ex_cline_download_control, 0, sizeof(ex_cline_download_control));
    init_download_variable();
    ex_cline_download_control.baudrate = 9600;

    return;
}
/*********************************************************************//**
 * @brief TMSG
 * @param[in]	None
 * @return 		None
 **********************************************************************/
void _id003_msg_proc(void)
{
	switch (cline_msg.tmsg_code)
	{
	case TMSG_UART01CB_CALLBACK_INFO:
		_id003_callback_msg_proc();
		break;
	case TMSG_CLINE_STATUS_INFO:
		_id003_status_info_msg_proc();
		break;
	case TMSG_CLINE_DOWNLOAD_RSP:
		_id003_download_rsp_msg_proc();
		break;
	case TMSG_CLINE_WRITE_FLASH_RSP:
		_id003_write_flash_rsp_msg_proc();
		break;
	case TMSG_CLINE_DOWNLOAD_CALC_CRC_RSP:
		_id003_download_calc_crc_rsp_msg_proc();
		break;
	default:					/* other */
		break;
	}
}


/*********************************************************************//**
 * @brief Status request receiving procedure
 * @param[in]	None
 * @return 		None
 **********************************************************************/
void _id003_sts_request_cmd_proc(void)
{
	if (s_id003_communication_stsreq_flag != 0)
	{
		_id003_send_host_commu_err();
		/* uart error */
		s_id003_communication_stsreq_flag = 0;
	}
	else
	{
		switch (ex_cline_status_tbl.line_task_mode)
		{
		case ID003_MODE_WAIT_REQ:
		case ID003_MODE_WAIT_DOWNLOAD_START_RSP:
		case ID003_MODE_DOWNLOAD_ILLEGAL_FILE_ERROR:
			_id003_stsreq_failure();
			break;
		case ID003_MODE_DOWNLOAD_ERROR:
			_id003_stsreq_write_rom_failure();
			break;
		case ID003_MODE_DOWNLOAD_COMPLETE_ERROR:
			_id003_stsreq_download_error();
			break;
		case ID003_MODE_DOWNLOAD_COMPLETE:
		case ID003_MODE_DOWNLOAD_WAIT_RESET:
			_id003_stsreq_download_end();
			break;
		case ID003_MODE_DOWNLOAD:
		case ID003_MODE_DOWNLOAD_BUSY:
		case ID003_MODE_DOWNLOAD_CALC_CRC:
			_id003_stsreq_download();
			break;
		case ID003_MODE_QUIT:
			break;
		default:
			/* system error ? */
			_cline_system_error(0, 8);
			break;
		}
	}
}

/*********************************************************************//**
 * @brief response status (ID003_STS_FAILURE)
 * @param[in]	None
 * @return 		None
 **********************************************************************/
void _id003_stsreq_failure(void)
{
	_id003_tx_buff.length = 2 + 4;
	_id003_tx_buff.cmd = ID003_STS_FAILURE;
	_id003_tx_buff.data[0] = ID003_FAILURE_EXT_ROM;

	uart_send_id003(&_id003_tx_buff);
}
/*********************************************************************//**
 * @brief response status (ID003_STS_COMMUNICATION_ERROR)
 * @param[in]	None
 * @return 		None
 **********************************************************************/
void _id003_send_host_commu_err(void)
{
	_id003_tx_buff.length = 1 + 4;
	_id003_tx_buff.cmd = ID003_STS_COMMUNICATION_ERROR;

	uart_send_id003(&_id003_tx_buff);
}

/*********************************************************************//**
 * @brief response status (ID003_STS_FAILURE)
 * @param[in]	None
 * @return 		None
 **********************************************************************/
void _id003_stsreq_write_rom_failure(void)
{
	_id003_tx_buff.length = 2 + 4;
	_id003_tx_buff.cmd = ID003_STS_FAILURE;
	_id003_tx_buff.data[0] = ID003_FAILURE_EXT_ROM_WRITE;

	uart_send_id003(&_id003_tx_buff);
}
/*********************************************************************//**
 * @brief response status (ID003_STS_DOWNLOAD_END)
 * @param[in]	None
 * @return 		None
 **********************************************************************/
void _id003_stsreq_download_error(void)
{
	_id003_tx_buff.length = 4 + 4;
	_id003_tx_buff.cmd = ID003_STS_DOWNLOAD_END;
	_id003_tx_buff.data[0] = ID003_DOWNLOAD_FAILURE;
	_id003_tx_buff.data[1] = (u8)(ex_cline_download_control.file_crc >> 8);
	_id003_tx_buff.data[2] = (u8)(ex_cline_download_control.file_crc & 0xFF);

	uart_send_id003(&_id003_tx_buff);
}
/*********************************************************************//**
 * @brief response status (ID003_STS_DOWNLOAD_END)
 * @param[in]	None
 * @return 		None
 **********************************************************************/
void _id003_stsreq_download_end(void)
{
	_id003_tx_buff.length = 4 + 4;
	_id003_tx_buff.cmd = ID003_STS_DOWNLOAD_END;
	_id003_tx_buff.data[0] = ID003_DOWNLOAD_SUCCESS;
	_id003_tx_buff.data[1] = (u8)(ex_cline_download_control.file_crc >> 8);
	_id003_tx_buff.data[2] = (u8)(ex_cline_download_control.file_crc & 0xFF);

	uart_send_id003(&_id003_tx_buff);

    /* wait */
    while(UART_is_txd_active())
    {
    	OSW_TSK_sleep(1);
    };
	ex_cline_status_tbl.line_task_mode = ID003_MODE_DOWNLOAD_WAIT_RESET;
	_cline_send_msg(ID_MAIN_MBX, TMSG_CLINE_SOFT_RESET_REQ, 0, 0, 0, 0);
}
/*********************************************************************//**
 * @brief response status (ID003_STS_DOWNLOAD)
 * @param[in]	None
 * @return 		None
 **********************************************************************/
void _id003_stsreq_download(void)
{
	if(ex_cline_download_control.busy)
	{
		_id003_tx_buff.length = 2 + 4;
		_id003_tx_buff.cmd = ID003_STS_DOWNLOAD;
		_id003_tx_buff.data[0] = ID003_DOWNLOAD_STS_BUSY;
	}
	else if(ex_cline_download_control.is_start_received == DL_START_NORMAL)
	{
		_id003_tx_buff.length = 2 + 4;
		_id003_tx_buff.cmd = ID003_STS_DOWNLOAD;
		_id003_tx_buff.data[0] = ID003_DOWNLOAD_STS_READY;
	}
	else if(ex_cline_download_control.is_start_received == DL_START_DIFFERENTIAL)
	{
		_id003_tx_buff.length = 6 + 4;
		_id003_tx_buff.cmd = ID003_STS_DOWNLOAD;
		_id003_tx_buff.data[0] = ID003_DOWNLOAD_STS_READY_DIFFERENTIAL;
		_id003_tx_buff.data[1] = (u8)((s_dl_file_offset & 0xFF)            );
		_id003_tx_buff.data[2] = (u8)((s_dl_file_offset & 0xFF00)     >> 8 );
		_id003_tx_buff.data[3] = (u8)((s_dl_file_offset & 0xFF0000)   >> 16);
		_id003_tx_buff.data[4] = (u8)((s_dl_file_offset & 0xFF000000) >> 24);
	}
	else
	{
		_id003_tx_buff.length = 2 + 4;
		_id003_tx_buff.cmd = ID003_STS_DOWNLOAD;
		_id003_tx_buff.data[0] = ID003_DOWNLOAD_STS_READY;
	}

	uart_send_id003(&_id003_tx_buff);
}

/*********************************************************************//**
 * @brief UART1 callback message receiving procedure
 * @param[in]	None
 * @return 		None
 **********************************************************************/
void _id003_callback_msg_proc(void)
{
	switch (cline_msg.arg1)
	{
	case TMSG_SUB_RECEIVE:
		_id003_callback_receive();
		break;
	case TMSG_SUB_EMPTY:
		_id003_callback_empty();
		break;
	case TMSG_SUB_ALARM:
		_id003_callback_alarm();
		break;
	default:					/* other */
		break;
	}
}
/*********************************************************************//**
 * @brief UART1 callback TXD message procedure
 * @param[in]	None
 * @return 		None
 **********************************************************************/
void _id003_callback_empty(void)
{
}
/*********************************************************************//**
 * @brief UART1 callback RXD message receiving procedure
 * @param[in]	None
 * @return 		None
 **********************************************************************/
void _id003_callback_receive(void)
{
	u8 rtn;
	rtn = uart_listen_id003(&_id003_rx_buff);
	if (rtn == ID003_LISTEN_COMMAND)
	{
	/* Receiving command */
		_id003_cmd_proc();
	}
}
/*********************************************************************//**
 * @brief UART1 callback error message receiving procedure
 * @param[in]	None
 * @return 		None
 **********************************************************************/
void _id003_callback_alarm(void)
{
	s_id003_communication_stsreq_flag = 1;
}



/*********************************************************************//**
 * @brief Command receiving procedure
 * @param[in]	_id003_rx_buff.cmd : command code
 * @return 		None
 **********************************************************************/
void _id003_cmd_proc(void)
{

	switch (_id003_rx_buff.cmd)
	{
	case ID003_CMD_STS_REQUEST:						/* Status Request (Poll) */
		_id003_sts_request_cmd_proc();
		break;
	case ID003_CMD_DOWNLOAD_REQUEST:				/* Download Request */
		_id003_download_start_cmd_proc();
		break;
	case ID003_CMD_DOWNLOAD_END_REQUEST:
		_id003_download_end_confirm_cmd_proc();
		break;
	case ID003_CMD_BAUDRATE_REQUEST:
		_id003_download_baudrate_cmd_proc();
		break;
	case ID003_CMD_DIFFERENTIAL_DOWNLOAD:
		_id003_diff_download_start_cmd_proc();
		break;
	case ID003_CMD_DOWNLOAD_DATA:
		_id003_download_data_cmd_proc();
		break;
	case ID003_CMD_ACK:
		_id003_ack_proc();
		break;
	default:
		_id003_send_host_invalid();
		break;
	}
}

/*********************************************************************//**
 * @brief Status infomation message receiving procedure
 * @param[in]	None
 * @return 		None
 **********************************************************************/
void _id003_status_info_msg_proc(void)
{
}
/*********************************************************************//**
 * @brief Download Request receiving procedure
 * @param[in]	None
 * @return 		None
 **********************************************************************/
void _id003_download_start_cmd_proc(void)
{
    /* ダウンロード関係変数の初期化 */
    _init_variable();
    /* ダウンロード開始要求フラグ     */
	ex_cline_download_control.is_start_received = DL_START_NORMAL;


    /* ID003ﾚｽﾎﾟﾝｽﾒｯｾｰｼﾞ作成(ACK) */
    _id003_send_host_ack();

    if(ex_cline_status_tbl.line_task_mode == ID003_MODE_WAIT_REQ)
    {
    	ex_cline_status_tbl.line_task_mode = ID003_MODE_WAIT_DOWNLOAD_START_RSP;
    	_cline_send_msg(ID_MAIN_MBX, TMSG_CLINE_DOWNLOAD_REQ, 0, 0, 0, 0);
    }
}
/*********************************************************************//**
 * @brief Download Request receiving procedure
 * @param[in]	None
 * @return 		None
 **********************************************************************/
void _id003_diff_download_start_cmd_proc(void)
{
    /* ダウンロード関係変数の初期化 */
    _init_variable();
    /* ダウンロード開始要求フラグ     */
	ex_cline_download_control.is_start_received = DL_START_DIFFERENTIAL;

    /* ID003ﾚｽﾎﾟﾝｽﾒｯｾｰｼﾞ作成(ACK) */
    _id003_send_host_ack();

    if(ex_cline_status_tbl.line_task_mode == ID003_MODE_WAIT_REQ)
    {
    	ex_cline_status_tbl.line_task_mode = ID003_MODE_WAIT_DOWNLOAD_START_RSP;
    	_cline_send_msg(ID_MAIN_MBX, TMSG_CLINE_DOWNLOAD_REQ, 0, 0, 0, 0);
    }
}
/*********************************************************************//**
 * @brief Download Request receiving procedure
 * @param[in]	None
 * @return 		None
 **********************************************************************/
void _id003_download_end_confirm_cmd_proc(void)
{
    if(ex_cline_status_tbl.line_task_mode == ID003_MODE_DOWNLOAD)
    {
		/*********************/
		/* BUSY状態にする                       */
		/*********************/
		ex_cline_download_control.busy = TRUE;
		ex_cline_status_tbl.line_task_mode = ID003_MODE_DOWNLOAD_CALC_CRC;

		_cline_send_msg(ID_MAIN_MBX, TMSG_CLINE_DOWNLOAD_CALC_CRC_REQ, 0, 0, 0, 0);
	    /* ID003ﾚｽﾎﾟﾝｽﾒｯｾｰｼﾞ作成(ACK) */
	    _id003_send_host_ack();
    }
    else
    {
		_id003_send_host_invalid();
    }
}
void _id003_get_baudrate_command(void)
{
	switch( ex_cline_download_control.baudrate )
	{
	case 9600:
		/* ID003ﾚｽﾎﾟﾝｽﾒｯｾｰｼﾞ作成(9600bps) */
		_id003_tx_buff.length = 2 + 4;
		_id003_tx_buff.cmd = ID003_CMD_BAUDRATE_REQUEST;
		_id003_tx_buff.data[0] = 0x01;
		uart_send_id003(&_id003_tx_buff);
		break;
	case 19200:
		/* ID003ﾚｽﾎﾟﾝｽﾒｯｾｰｼﾞ作成(19200bps) */
		_id003_tx_buff.length = 2 + 4;
		_id003_tx_buff.cmd = ID003_CMD_BAUDRATE_REQUEST;
		_id003_tx_buff.data[0] = 0x02;
		uart_send_id003(&_id003_tx_buff);
		break;
	case 38400:
		/* ID003ﾚｽﾎﾟﾝｽﾒｯｾｰｼﾞ作成(38400bps) */
		_id003_tx_buff.length = 2 + 4;
		_id003_tx_buff.cmd = ID003_CMD_BAUDRATE_REQUEST;
		_id003_tx_buff.data[0] = 0x03;
		uart_send_id003(&_id003_tx_buff);
		break;
	case 57600:
		/* ID003ﾚｽﾎﾟﾝｽﾒｯｾｰｼﾞ作成(57600bps) */
		_id003_tx_buff.length = 2 + 4;
		_id003_tx_buff.cmd = ID003_CMD_BAUDRATE_REQUEST;
		_id003_tx_buff.data[0] = 0x04;
		uart_send_id003(&_id003_tx_buff);
		break;
	case 115200:
		/* ID003ﾚｽﾎﾟﾝｽﾒｯｾｰｼﾞ作成(115200bps) */
		_id003_tx_buff.length = 2 + 4;
		_id003_tx_buff.cmd = ID003_CMD_BAUDRATE_REQUEST;
		_id003_tx_buff.data[0] = 0x05;
		uart_send_id003(&_id003_tx_buff);
		break;
	default:
		break;
	}
}
void _id003_set_baudrate_command(void)
{
    /* ID003ﾚｽﾎﾟﾝｽﾒｯｾｰｼﾞ作成(ACK) */
    _id003_send_host_ack();

    /* wait */
    while(UART_is_txd_active())
    {
    	OSW_TSK_sleep(1);
    };

	switch( _id003_rx_buff.data[1] )
	{
	/* 通信ﾚｰﾄ : 9600bps   */
	case 0x01:
		ex_cline_download_control.baudrate = 9600;
		break;
	/* 通信ﾚｰﾄ : 19200bps  */
	case 0x02:
		ex_cline_download_control.baudrate = 19200;
		break;
	/* 通信ﾚｰﾄ : 38400bps  */
	case 0x03:
		ex_cline_download_control.baudrate = 38400;
		break;
	/* 通信ﾚｰﾄ : 57600bps  */
	case 0x04:
		ex_cline_download_control.baudrate = 57600;
		break;
	/* 通信ﾚｰﾄ : 115200bps */
	case 0x05:
		ex_cline_download_control.baudrate = 115200;
		break;
	/* 通信ﾚｰﾄ : 9600bps   */
	default:
		ex_cline_download_control.baudrate = 9600;
		break;
	}
	UART_change_baudrate(ex_cline_download_control.baudrate);
}
/*********************************************************************//**
 * @brief Download Request receiving procedure
 * @param[in]	None
 * @return 		None
 **********************************************************************/
void _id003_download_baudrate_cmd_proc(void)
{
    if( _id003_rx_buff.data[0] == 0x00 )
    {
        _id003_get_baudrate_command();
    }
    /* Setコマンドの場合 */
    else if( _id003_rx_buff.data[0] == 0x01 )
    {
        _id003_set_baudrate_command();
    }
}
/*********************************************************************//**
 * @brief Download Request receiving procedure
 * @param[in]	None
 * @return 		None
 **********************************************************************/
void _id003_download_data_cmd_proc(void)
{
    if(ex_cline_status_tbl.line_task_mode == ID003_MODE_DOWNLOAD)
    {
        /************************/
        /* Downlaodデータコピー */
        /************************/
        download_copy( &(_id003_rx_buff.data[4]), _id003_rx_buff.length-9 );

        /*********************/
        /* Downlaodチェック */
        /*********************/
        if(RET_OK == download_data_check())
    	{

			#if 1 //2023-12-22 ID-003 ダウンロード最初の1回のみにする
			if(ex_download_start_id003 == 0)
			{
				ex_download_start_id003 = 1;
				_cline_send_msg(ID_DISPLAY_MBX, TMSG_DISP_LED_DOWNLOAD_ING, 0, 0, 0, 0);
			}
			#endif

            /*********************/
            /* BUSY状態にする                       */
            /*********************/
            ex_cline_download_control.busy = TRUE;
        	ex_cline_status_tbl.line_task_mode = ID003_MODE_DOWNLOAD_BUSY;

        	_cline_send_msg(ID_MAIN_MBX, TMSG_CLINE_WRITE_FLASH_REQ, 0, 0, 0, 0);
    	    /* ID003ﾚｽﾎﾟﾝｽﾒｯｾｰｼﾞ作成(ACK) */
    	    _id003_send_host_ack();
    	}
    	else
    	{
    		_id003_send_host_invalid();
    	}
    }
    else
    {
		_id003_send_host_invalid();
    }
}

/*********************************************************************//**
 * @brief Receive ACK
 * @param[in]	None
 * @return 		None
 **********************************************************************/
void _id003_ack_proc(void)
{
}
/*********************************************************************//**
 * @brief Send ACK
 * @param[in]	None
 * @return 		None
 **********************************************************************/
void _id003_send_host_ack(void)
{
	_id003_tx_buff.length = 1 + 4;
	_id003_tx_buff.cmd = ID003_CMD_ACK;
	uart_send_id003(&_id003_tx_buff);
}


/*********************************************************************//**
 * @brief Send Invalid
 * @param[in]	None
 * @return 		None
 **********************************************************************/
void _id003_send_host_invalid(void)
{
	_id003_tx_buff.length = 1 + 4;
	_id003_tx_buff.cmd = ID003_CMD_INVALID;
	uart_send_id003(&_id003_tx_buff);
}


/*********************************************************************//**
 * @brief Download response message receiving procedure
 * @param[in]	None
 * @return 		None
 **********************************************************************/
void _id003_write_flash_rsp_msg_proc(void)
{
    if(ex_cline_status_tbl.line_task_mode == ID003_MODE_DOWNLOAD_BUSY)
    {
        ex_cline_download_control.busy = FALSE;
    	if (cline_msg.arg1 == TMSG_SUB_SUCCESS)
    	{
    		// End Write Flash
    		ex_cline_status_tbl.line_task_mode = ID003_MODE_DOWNLOAD;
    	}
    	else if (cline_msg.arg1 == TMSG_SUB_ALARM)
    	{
    		id_abn_set(ALARM_CODE_ROM_WRITE);
    		// Download request refuse
    		ex_cline_status_tbl.line_task_mode = ID003_MODE_DOWNLOAD_ERROR;
    	}
    	else
    	{
    		/* system error ? */
    		_cline_system_error(0, 10);
    	}
    }
	else
	{
		/* system error ? */
		_cline_system_error(0, 10);
	}
}

/*********************************************************************//**
 * @brief Download response message receiving procedure
 * @param[in]	None
 * @return 		None
 **********************************************************************/
void _id003_download_calc_crc_rsp_msg_proc(void)
{
    if(ex_cline_status_tbl.line_task_mode == ID003_MODE_DOWNLOAD_CALC_CRC)
    {
        /* Ready状態にする        */
        ex_cline_download_control.busy = FALSE;
    	if (cline_msg.arg1 == TMSG_SUB_SUCCESS)
    	{
            /* ダウンロード正常完了状態 */
        	s_dl_end_status = D_DL_COMPLENTION;
            /* ダウンロード正常完了状態 */
        	ex_cline_status_tbl.line_task_mode = ID003_MODE_DOWNLOAD_COMPLETE;
    	}
    	else if (cline_msg.arg1 == TMSG_SUB_ALARM)
    	{
    		id_abn_set(ALARM_CODE_DOWNLOAD);
            /* ダウンロード失敗状態 */
        	s_dl_end_status = D_DL_FAILURE;
            /* ダウンロード失敗状態 */
        	ex_cline_status_tbl.line_task_mode = ID003_MODE_DOWNLOAD_COMPLETE_ERROR;
    	}
    	else
    	{
    		/* system error ? */
    		_cline_system_error(0, 10);
    	}
    }
	else
	{
		/* system error ? */
		_cline_system_error(0, 10);
	}
}
/*********************************************************************//**
 * @brief Download response message receiving procedure
 * @param[in]	None
 * @return 		None
 **********************************************************************/
void _id003_download_rsp_msg_proc(void)
{
    if(ex_cline_status_tbl.line_task_mode == ID003_MODE_WAIT_DOWNLOAD_START_RSP)
    {
    	if (cline_msg.arg1 == TMSG_SUB_SUCCESS)
    	{
    		// Start I/F Download
    		ex_cline_status_tbl.line_task_mode = ID003_MODE_DOWNLOAD;
    	}
    	else if (cline_msg.arg1 == TMSG_SUB_ALARM)
    	{
    		// Download request refuse
    		ex_cline_status_tbl.line_task_mode = ID003_MODE_QUIT;
    	}
    	else
    	{
    		/* system error ? */
    		_cline_system_error(0, 10);
    	}
    }
	else
	{
		/* system error ? */
		_cline_system_error(0, 10);
	}
}

/* EOF */

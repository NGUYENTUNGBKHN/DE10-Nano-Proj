...
{
	/* WDT0 Peripheral Reset */
	RSTMGR_PERMODRST_T permodrst = {0};
	do {
		permodrst.lword = IOREG32(RSTMGR_BASE, RSTMGR_PERMODRST);		// 内蔵モジュールリセット制御レジスタ読出し
		permodrst.bit.l4wd0 = 1;		// WDT0モジュールリセットアサート
		IOREG32(RSTMGR_BASE, RSTMGR_PERMODRST) = permodrst.lword;		// 内蔵モジュールリセット制御レジスタ書込み
		__nop();
		permodrst.lword = IOREG32(RSTMGR_BASE, RSTMGR_PERMODRST);		// 内蔵モジュールリセット制御レジスタ読出し
	} while (permodrst.bit.l4wd0 != 1);		// リセットアサート待ち
	do {
		permodrst.lword = IOREG32(RSTMGR_BASE, RSTMGR_PERMODRST);		// 内蔵モジュールリセット制御レジスタ読出し
		permodrst.bit.l4wd0 = 0;		// WDT0モジュールリセットデアサート
		IOREG32(RSTMGR_BASE, RSTMGR_PERMODRST) = permodrst.lword;		// 内蔵モジュールリセット制御レジスタ書込み
		__nop();
		permodrst.lword = IOREG32(RSTMGR_BASE, RSTMGR_PERMODRST);		// 内蔵モジュールリセット制御レジスタ読出し
	} while (permodrst.bit.l4wd0 != 0);		// リセットデアサート待ち
}

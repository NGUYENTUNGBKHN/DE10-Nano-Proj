...
#define __HAL_USB2_RESET	(GPIO_24)
#define __HAL_FUSB_SELECT	(GPIO_25)
#define __HAL_SU_SELECT		(GPIO_27)
#define __HAL_IF_SEL2		(GPIO_28) //cc-talkへの切り替え Hihg ID-003, Low-cc-talk
...
//	//GIPOの番号で確認、excell資料準拠 UBAで使用していない番号の場合保留
	...
	#define __HAL_PB_HOME		(GPI_02)	/* PB Homeの*/
	#define __HAL_PB_ENC		(GPI_01)	/* PB HomeのENC*/

	#define __HAL_HOME_LED		(GPIO_52)	/* Sub LED*/

	#define __HAL_SHUTTER_HOME	(GPIO_18)	/* Shutter Home */

	#if defined(UBA_RTQ)
	#define GPIO_RC_RESET		(GPIO_66)	/* RC RESET */// LowでRTQに対してReset, HighでReset解除
	#endif

	#define __HAL_5V_PG_N		(GPIO_17) //後ろの電源を検知
...
#define	GPIO_RESETB1		(__HAL_USB2_RESET)
...
#define GPIO_EXT_RESET		(__HAL_DET_RES)
...
/*						IF LED信号								*/
/****************************************************************/
#define GPIO_IF_SEL2	(__HAL_IF_SEL2) //cc-talkへの切り替え Hihg ID-003, Low-cc-talk

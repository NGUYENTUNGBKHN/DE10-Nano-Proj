...
#if 1//	/* 2022-05-26 */
#define TCA9535_0_DIR0		0x03	// 10000011B	/* 1:in/0:out */
#else
...
/* I/O expanders polarity data */
#if 1//	/* 2022-05-26 */
#define TCA9535_0_POL0		0x00	// 10000011B	/* 1:inverted/0:retained */
#else
...
#if MAG1_ENABLE
#define BIT_MAGCAL		0x80
#define BIT_MAGCNTL		0x40
#endif
/************************** PRIVATE VARIABLES *************************/

/************************** PRIVATE FUNCTIONS *************************/
static INT32 _hal_i2c3_init_tca9535(u8 devno);
/************************** EXTERN VARIABLES *************************/
/************************** EXTERN FUNCTIONS *************************/
extern u8 _pl_ioex_reset(u8 set);
void __hal_i2c3_por(void)
{
	_pl_ioex_reset(1);
	OSW_TSK_sleep(10);
	_pl_ioex_reset(0);
 ...
 * @retval      OTHER failed
 **********************************************************************/
static INT32 _hal_i2c3_tca9535_write_pol1(u8 data)
{
	I2C_SEND_PACK pack;
	u8 w[16];
	INT32 ercd = E_OK;
 ...
 * @retval      OTHER failed
 **********************************************************************/
static INT32 _hal_i2c3_tca9535_write_dir1(u8 data)
{
	I2C_SEND_PACK pack;
	u8 w[16];
	INT32 ercd = E_OK;
...
INT32 _hal_i2c3_read_dipsw1(UINT8 *dipsw_value)
{
	...
	ercd =_hal_i2c3_tca9535_write_pol1(TCA9535_0_POL1);
	//2025-02-21
	if(ercd != E_OK)
	{
		return ercd;
	}
	ercd =_hal_i2c3_tca9535_write_dir1(TCA9535_0_DIR1);
	if(ercd != E_OK)
	{
		return ercd;
	}
...
INT32 _hal_i2c3_write_mag_cal(UINT8 data)
{
	INT32 ercd = E_OK;

//	ercd = _hal_i2c3_tca9535_set_dir(data, BIT_MAGCAL);
	ercd = _hal_i2c3_read_write_p0(data, BIT_MAGCAL);

	return ercd;
}

INT32 _hal_i2c3_write_mag_cntl(UINT8 data)
{
	INT32 ercd = E_OK;

//	ercd = _hal_i2c3_tca9535_set_dir(data, BIT_MAGCNTL);
	ercd = _hal_i2c3_read_write_p0(data, BIT_MAGCNTL);

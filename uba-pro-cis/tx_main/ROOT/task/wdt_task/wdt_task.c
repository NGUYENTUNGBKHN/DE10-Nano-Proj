...
void wdt_init(void)
{
	...
	//割り込み周期とタイムアウトの時間は同じ時間しか設定できない
	//5.3s周期で割込みが発生し、その後5.3s以内にカウンタを再設定しないとResetになる
	//その為、考え方としては5.3s以上10.6s以下でResetとなる。
	...
	alt_wdog_counter_set(ALT_WDOG1, ALT_WDOG_TIMEOUT128M);		// WDTリピートカウンタ設定(25MHz) //5.3s設定
#else
	alt_wdog_counter_set(ALT_WDOG_INIT, ALT_WDOG_TIMEOUT32M);		// WDTイニシャルカウンタ設定(32Mカウント / 25MHz = 1.342s)
	alt_wdog_counter_set(ALT_WDOG1, ALT_WDOG_TIMEOUT32M);		// WDTリピートカウンタ設定(32Mカウント / 25MHz = 1.342s)
#endif

	alt_wdog_response_mode_set(ALT_WDOG1, ALT_WDOG_INT_THEN_RESET);		// 割込み->ウォームリセット設定
	...
	alt_wdog_start(ALT_WDOG1);		// WDTスタート
}
...
void wdt_clear(void)
{
	alt_wdog_reset(ALT_WDOG1);		// WDTカウントリセット
...
#if 0	//2023-06-28 テスト的にウオッチドック禁止 2023-07-22
	while (true) 
	{

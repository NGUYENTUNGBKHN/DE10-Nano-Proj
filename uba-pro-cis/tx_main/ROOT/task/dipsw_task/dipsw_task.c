...
void dipsw_task_initialize(void)
{
	...
	if (ercd == E_OK)
	{
		...
		{
			_dipsw_send_msg(ID_MAIN_MBX, TMSG_DIPSW_INIT_RSP, TMSG_SUB_SUCCESS, ex_dipsw2, 0, 0);
		}
	}
...
void dipsw_port_initialize(void)
{
	...
	ercd = _hal_read_dipsw1(&l_dipsw); //2025-02-21 ここで確実にDIP-SWがよめるのを確認して、DIP-SWのフラグを立てた方がいい
	//2025-02-21 ここでは完全に読み取り成功までループさせる
	if(ercd != E_OK)
	{
		do{
			dly_tsk(20);
...
void _dipsw_idle_msg_proc(void)
{
	...
	//not use	_dipsw_send_msg(ID_MAIN_MBX, TMSG_DIPSW_INIT_RSP, TMSG_SUB_SUCCESS, ex_dipsw1, 0, 0);
		break;
	default:
		/* system error ? */
...
void _dipsw_system_error(u8 fatal_err, u8 code)
{
	...
	if (fatal_err)
	{
		_dipsw_send_msg(ID_DISPLAY_MBX, TMSG_DISP_LED_ALARM, ALARM_CODE_TASK_AREA, 0, 0, 0);
	}

...
void _intr_uart_receive_id003( UINT8 data, UINT8 stat )
{
	...
	case 4: /* CRC (H) */
		...
		{
			#if defined(UBA_RTQ)	//#if defined(UBA_ID003_ENC) //暗号化 16byte, SPECK用
			/* CRC error but passed. Because CRC was exactly calculated but after was be encrypted  */
			if(s_uart_info_id003->ring_buff.rx[2] == 0x10 && 
				(s_uart_info_id003->ring_buff.rx[1] == 0x0b || s_uart_info_id003->ring_buff.rx[1] == 0x13))
			{
				s_uart_info_id003->ring_buff.rx_tail = s_id003_rx_len;
				iset_flg(ID_UART01_CB_FLAG, EVT_UART_RCV);
			}
...
u8 uart_send_plain_id003(ID003_CMD_BUFFER *txbuf)	// ID-003 暗号化通信用 //2024-09-11 CRCは計算済みなので、データをセットするだけ
{
	s32 rtn = 0;
	u32 bytes = txbuf->length;
	u32 cnt;

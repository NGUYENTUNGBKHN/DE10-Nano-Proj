...
static void spi_stb_assert( SPI_STR *pSpi, UINT8 ch )
{
	...
	UINT32 bit = 1;
	UINT32 ser;
	ser = IOREG32(pSpi->base,SPI_SER);
	if( CFG_SPIM_FMT[n] & SPIM_FMT_STB_H_ACTIVE ){
		IOREG32(pSpi->base,SPI_SER) = (ser | (bit<<ch));
	}
	else {
		IOREG32(pSpi->base,SPI_SER) = (ser & ~(bit<<ch));
	}
...
static INT8 spi_reg_init( SPI_STR *pSpi )
{
	...
	// jcm mod (TI to MOTO)
	IOREG32(pSpi->base,SPI_CTRLR0) = 0x07;
	//IOREG32(pSpi->base,SPI_CTRLR0) = 0x27;
	...
	// jcm add
	IOREG32(pSpi->base,SPI_RX_SAMPLE_DRY) = 0x1;
...
INT8 Spi_send( SPI_HANDLE *handle, UINT8 ch, SPIM_SEND *send )
{
	...
	if( send->len ){
		...
		if(send->recv == NULL)
		{
			while( (pSpi->mst.slen < pSpi->mst.send.len) )
			{
				if(0xff == IOREG32(pSpi->base,SPI_TXFLR))
				{
					// full
					break;
				}
				...
				w = 0;
				pSpi->mst.tx_pend = 0;
				if( pSpi->mst.send.send ){
					w = pSpi->mst.send.send[(pSpi->mst.slen&(~0x3))+pSpi->mst.tbl[(pSpi->mst.slen&0x3)]];
				}
				pSpi->mst.slen++;
				IOREG32(pSpi->base,SPI_DR) = w;
			}
			if( pSpi->mst.slen >= pSpi->mst.send.len)
			{

...
uint8_t clock_enabled[ALT_CLK_H2F_USER2+1];
uint32_t clock_frequency[ALT_CLK_H2F_USER2+1];
uint8_t safe_enabled[2];
...
void set_main_pll(alt_freq_t new_freq)
{
	...
	alt_freq_t cur_freq;
	float div;
	uint32_t main_multi,main_div;
	uint32_t cur_multi,cur_div;
	uint32_t new_multi,new_div;
...
//			program_error();
    ...
    if (status == ALT_E_SUCCESS)
    {
		...
		//clock 200mhz to 1mhz
		alt_clk_divider_set(ALT_CLK_GPIO_DB, 200);
    }
}
/*********************************************************************//**
 * @brief		Control clk LED
...
void get_pll_info(void)
{
	...
	uint32_t main_multi,main_div;
	uint32_t peri_multi,peri_div;
	uint32_t sdram_multi,sdram_div;
	// get freq
	alt_clk_pll_vco_cfg_get(ALT_CLK_MAIN_PLL,&main_multi,&main_div);
	alt_clk_pll_vco_cfg_get(ALT_CLK_PERIPHERAL_PLL,&peri_multi,&peri_div);
	alt_clk_pll_vco_cfg_get(ALT_CLK_SDRAM_PLL,&sdram_multi,&sdram_div);
	for(i = (uint32_t)ALT_CLK_MAIN_PLL; i <= (uint32_t)ALT_CLK_H2F_USER2; i++)
	{

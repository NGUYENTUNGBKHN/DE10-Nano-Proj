; ------------------------------------------------------------
; Cortex-A9 MPCore SMP Fireworks Example
;
...
; Porting defines
; ------------------------------------------------------------
PABASE_MPU        EQU   0xA0410000  ; PA base of MPU and L2CC  (1MB align)
;PABASE_CM         EQU   0x4A000000  ; PA base of Clock subsystem (1MB align)
PABASE_PERI3      EQU   0x80000000  ; PA base of GPIOs 128-159 and UART2 (1MB align)

L2CC_PL310        EQU   0xA0412000

L1_COHERENT       EQU   0x00015c06  ; Template descriptor for coherent memory (and L2 cache enabled)
...
IRQ_Handler   PROC {r0-r12}
  ...
  CMP   r4, #29 ; Private Timer interrupt
unhandled_irq
  BNE     unhandled_irq
...
ttb_zero_loop
  ...
  ;
  ; Additional entries for peripheral registers
  LDR     r1, =PABASE_PERI3         ; Physical address range for Display subsystem and UART memory space
  ...
  LDR     r1, =PABASE_MPU           ; Physical address for MPU
  LSR     r1, r1, #20
  LSL     r2, r1, #2
  LSL     r1, r1, #20
  LDR     r3, =L1_DEVICE
  ...
  ;LDR     r1, =PABASE_CM            ; Physical address for CM subsystem registers
  ;LSR     r1, r1, #20
  ;LSL     r2, r1, #2
  ;LSL     r1, r1, #20
  ;LDR     r3, =L1_DEVICE
  ...
  LDR     r1, =||Image$$FRAMEBUFFER$$Base|| ; Physical address for Frame Buffer
  LSR     r1, r1, #20
  LSL     r2, r1, #2
  LSL     r1, r1, #20
  LDR     r3, =L1_DEVICE

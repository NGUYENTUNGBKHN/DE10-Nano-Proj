...
extern unsigned int tick_count;  // in timer_interrupts.c

#define FRAME_BUFFER 0x80500000
...
static void fireworks(void)
{
    ...
    while (1)
    {
//        tick_count=0; // Reset millisecond timer //  Add me in Step 3
        for (i = 0; i < 10000; i++)
        {
            drawSparks();
        }
//        printf("Time (millisecs) = %u\n", tick_count );  //  Add me in Step 3
    }
}
...
extern void i2c_init(void); // in i2c.c
extern void i2c_write(char sa, char addr, char value); // in i2c.c
...
//i2c address for TPS65950 (GPIO2 bank)
#define SCDA_GPIO 0x49

#define CONTROL_PRODUCTION_ID_BASE 0x4830A214
#define CONTROL_PRODUCTION_ID (volatile unsigned int *)(CONTROL_PRODUCTION_ID_BASE)
...
int main(void)
{
	...
	//Read the production id base to detect a BeagleBoard-xM
	id = *CONTROL_PRODUCTION_ID;

	if (id == DM37x_PROD_CODE) /* BeagleBoard-xM */
	{
	    //Enable the DVI_UP via TPS65950 GPIO2 using i2c protocol.
		i2c_init(); // in i2c.c
		/* Set port direction as output */
		i2c_write(SCDA_GPIO, GPIODATADIR1, GPIO2_ON); // in i2c.c
		/* Set port data '1' */
		i2c_write(SCDA_GPIO, GPIODATAOUT1, GPIO2_ON); // in i2c.c
	}
